...
void main() {
  group('fetchGuidanceAt', () {
    ...
    test('発行した照会の識別子を厳密キーと粗いキーの対で通知する（検索間の重複計測）', () async {
      // guidanceDupCalls は**1検索内**の重複しか数えない（クライアントは検索1回で作り捨て）。
      // 検索間・ユーザー間のヒット率はログを跨いで集計するしかないので、正規化キーを外へ出す。
      // 粗いキー（座標4桁・時刻10分バケット）を併記するのは、キャッシュキーをどこまで緩めれば
      // ヒットが増えるかがキャッシュ実装可否そのものになるため。
      final keys = <(String, String)>[];
      final client = TransitApiClient(
        transitClient: MockClient((req) async => _json({'ok': true})),
        proxyClient: MockClient((req) async => _json({'ok': true})),
        transitBaseUrl: _transitBase,
        proxyBaseUrl: _proxyBase,
        onGuidanceIssued: (raw, coarse) => keys.add((raw, coarse)),
      );
      await client.fetchGuidanceAt(
        const GeoPoint(35.123456, 139.234567),
        const GeoPoint(35.3, 139.4),
        DateTime(2026, 6, 27, 9, 7),
      );
      expect(keys, hasLength(1));
      expect(
        keys.single.$1,
        '35.123456,139.234567|35.300000,139.400000|20260627|09:07|train',
      );
      expect(
        keys.single.$2,
        '35.1235,139.2346|35.3000,139.4000|20260627|09:00|train',
        reason: '座標4桁・時刻を10分バケットへ丸めた粗いキー',
      );
    });

    test('粗いキーの時刻バケットは切り下げ・バス許容はモードで分かれる', () async {
      final keys = <(String, String)>[];
      final client = TransitApiClient(
        transitClient: MockClient((req) async => _json({'ok': true})),
        proxyClient: MockClient((req) async => _json({'ok': true})),
      ...
      const from = GeoPoint(35.0, 139.0);
      const to = GeoPoint(35.5, 139.5);
      await client.fetchGuidanceAt(from, to, DateTime(2026, 6, 27, 9, 19));
      await client.fetchGuidanceAt(
        from,
        to,
        DateTime(2026, 6, 27, 9, 11),
        allowBus: true,
      );
      expect(keys[0].$2.endsWith('|20260627|09:10|train'), isTrue);
      // 09:19 と 09:11 は同じ10分バケットへ落ちるが、モードが違えば別キー。
      expect(keys[1].$2.endsWith('|20260627|09:10|bus'), isTrue);
    });

    test('発行しなかった照会は通知しない（キャンセル・締切切れ）', () async {
      // 計上（onIssued）と同じ境界に揃える。往復していない要求をキーへ出すと、
      // 検索間ヒット率の分母が実際の上流負荷とずれる。
      final keys = <(String, String)>[];
