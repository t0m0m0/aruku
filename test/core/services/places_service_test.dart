...
void main() {
  ...
  group('GooglePlacesService.nearbySearch', () {
    test('action=textsearch で input と現在地 lat/lon を付与する', () async {
      late Uri captured;
      final client = MockClient((request) async {
        captured = request.url;
        return _jsonResponse({
      ...
      await service.nearbySearch(
        'マクドナルド',
        bias: const GeoPoint(35.681, 139.767),
      );

      expect(captured.queryParameters['action'], 'textsearch');
      expect(captured.queryParameters['input'], 'マクドナルド');
      expect(captured.queryParameters['lat'], '35.681');
      expect(captured.queryParameters['lon'], '139.767');
    });

    test('同梱座標を PlacePrediction.latLng に詰めて返す', () async {
      final client = MockClient(
        (_) async => _jsonResponse({
          'status': 'OK',
          'predictions': [
      ...
      final results = await service.nearbySearch(
        'マクドナルド',
        bias: const GeoPoint(35.68, 139.76),
      );

      expect(results, hasLength(1));
      expect(results.first.placeId, 'id_mac');
      expect(results.first.name, 'マクドナルド 東京駅店');
      expect(results.first.latLng, const GeoPoint(35.681, 139.767));
    });
    ...
    test('API エラーステータスで PlacesException をスローする', () async {
      ...
      expect(
        () => service.nearbySearch('test', bias: const GeoPoint(35.68, 139.76)),
        throwsA(isA<PlacesException>()),
      );
    });
