...
- **非単調コリドーの取りこぼし対策（#137）:** 二分探索は「到着が index 単調増」を仮定するが、実街路の前半徒歩は非単調になり得る（後方の停車駅が origin に近い等。実機: 川崎(徒歩74)より後方の鹿島田(徒歩68)の方が早着）。境界 index だけを採ると、二分探索が途中で評価した「より手前で徒歩の多い予算内点」を取りこぼす。よって**境界ではなく、二分探索が実測評価した点（メモ化済み）の中で予算内・徒歩最大の候補を返す**（同点は実到着が早い方）。追加 API は無い（評価済みのみ参照）。これは特定 OD に依存しない一般の改善で、`maxWalkBoardingIndex` の単調性仮定（§5）の suboptimality を緩和する。
