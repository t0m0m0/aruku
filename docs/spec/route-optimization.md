...
| 5 | 1 | 0 | 123.0s | 20.5s | 9.5s | **58.2s** | 34.7s | 1 | 36/11 | 1/1 | 28862→28649（−213ms） |

\#5 は `enrich*` 込みで測った最初の1本: `enrichCriticalMs=4872 enrichPasses=3 enrichResolveDepth=1 enrichCandidates=10`。**臨界パスが `enrichMs` の 8.4%** で、残りは縮退パス（best-effort＋バス last-resort＋再帰）＝計装外。probe 内直列の差は4本すべてで `boardSearchMs` の 0.6〜3.9%。
...
- **フェーズの支配要因は経路依存:** 崩壊時は `boardSearchMs`、通常時は `enrichMs` が支配する。片方の実測だけで「検索が遅い原因」を一般化しない。
  ...
  - **【2026-07-28 訂正】上の段数モデルは縮退しない検索にしか当てはまらない。** `enrichCriticalMs`（§3.8）を実装して初めて測った1本が反証した: `enrichResolveDepth=1` / `enrichCriticalMs=4.9s` に対し **`enrichMs=58.2s`**＝臨界パスはフェーズの **8.4%** しかなく、53.3s が計装外だった。内訳は `tier実測 ×2 → best-effort 縮退 → バス last-resort(guidance 1本) → _selectAndEnrich の再帰`。**予算内候補が実測で全滅して縮退へ落ちる検索では、支配するのは短リストの段数ではなく縮退パスそのもの。**
    - **`_bestEffortResolved` は測定口（`_measureCandidate`）を通らず `_resolveBoardingTimes` を直に呼び、短リストではなく候補プール全体を解決する**（`_maxMeasureShortlist`=13 の上限が効かない）。実測 `guidanceCalls=52` の大半がここ。次に計測を足すならこの経路（現状 `enrichCriticalMs` に入らない）。
    - **教訓:** #315 が `_measureCandidate` を「単一の測定口」と呼ぶのは**二重計測を防ぐため**で、全経路がそこを通る保証ではない。計装をそこへ寄せる前に他の経路の有無を確認すること。
  - **縮退しない検索では** `enrichMs` は短リストの「時刻なし transit 区間の段数」× 上流レイテンシで決まる（勝者の構成ではない）。`_resolveBoardingTimes` は区間ごとに guidance を**直列**に積むため（後続区間の `boardAt` が前区間の解決結果に依存・正当な依存）、1段＝上流1本ぶんの壁時計になる。n=4 の内訳がこれで説明できる：短リストが標準乗換中心（実 `depTime` 持ち→即 `continue`）だと **1.5s**、時刻なしハイブリッドが並ぶと **36.8〜55.7s**。**勝者が全徒歩でも短リストを測る費用は払う**（実測: 全徒歩が勝った検索で 36.8s）。
