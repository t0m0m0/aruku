...
- トグル切替 → `PlacesNotifier.setNearby(value)` が**取得済みの候補をその場で距離順へ再ソート**する。距離は通常検索でも各候補に付いているため、**追加の API リクエストは発行しない**（課金と debounce 400ms の待ちが要らない・§4）。関連度順の生結果は `_rawSuggestions` に保持し、OFF へ戻すと元の並びへ復帰する。
