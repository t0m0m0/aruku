...
**結果としての最悪待ち時間:** 探索は 120 秒で頭打ち。その後に走る徒歩実測は確定候補の**検証だけ**（探索側は締切でラウンドごと止まっているため）で、本数は候補の区間数（並列）× `_maxEnrichAttempts` で頭打ち。**厳密な 120 秒の天井ではない**が、この裾は #300 以前から存在した検証コストそのもので、今回の変更が増やしたものではない。**天井を締めたいなら検証を削るのではなく `_maxEnrichAttempts` を下げること。**
