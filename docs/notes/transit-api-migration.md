...
| 徒歩プロキシ1本 | **15 秒**（据置） | ハング検出 | Cloud Functions 経由の Google Routes は同じ裾を持たない。無応答は本当に異常＝早く縮退した方が良い |
| 検索の探索（Transit 引き直し） | **120 秒** | 体感保証 | 正常時 30〜60 秒の上に裾（30 秒級）が数本重なっても引き直しを完走できる幅を残し、最悪をそこで止める |
...
**天井の掛け方:** `SearchDeadline` を `TransitApiClient` に渡し、**Transit の各 fetch を `min(1本の上限, 残予算)` でクランプ**する（`_getOrTimeout`）。新ラウンドを止めるだけでは最悪が「締切＋1本の上限」＝155 秒になる。残予算 0 での照会は HTTP を発行せず即 `TIMEOUT`（必ず打ち切られる照会で無料の上流を叩かない）。

### 8.2 締切を掛けてよい呼び出し／いけない呼び出し（#300 レビュー指摘・重要）

**徒歩プロキシには締切を適用しない。** 当初は全 fetch を一律クランプしていたが、レビュー（PR #302）で**確定経路の徒歩実測まで飛ばして予算超過の経路を確定させる**ことが判明し、再現テストで確認した（実機相当の fixture で「23 分」と名乗る**実際 46 分**の全徒歩を予算 35 分内として確定）。

判断基準は速度ではなく**失敗の向き**：

| 呼び出し | 失敗の向き | 締切 | 理由 |
|---|---|---|---|
| Transit 引き直し（`/guidance/plan`） | **fail-closed** | **適用する** | 失敗すると候補が `unverified` として除外され、確証ある候補へ縮退する（§4 #137 approach A）。切っても**提示内容は嘘にならない** |
| 徒歩実測（Google Routes プロキシ） | **fail-open** | **適用しない** | `_enrichWalkGeometry` は取得失敗時に元の見積り（guidance / 直線）を**そのまま残す**。直線は実街路に対し大きく楽観に倒れる（実機で **-36 分・25%**）ため、切ると予算超過・乗り遅れの経路を平然と提示する（#254 の不変条件を破る） |

**measure-first の実測は探索の「改善」ではなく確定経路の「検証」であり、締切より優先する。**「遅いから諦める」でよいのは探索だけで、検証を諦めると**答えが劣化するのではなく嘘になる**。

**結果としての最悪待ち時間:** 探索は 120 秒で頭打ち。その後に走る徒歩実測は確定候補の**検証だけ**（探索側は締切でラウンドごと止まっているため）で、本数は短リスト（`_maxMeasureShortlist`）を候補**間並列**で測る 1〜2 バッチ（#315）で頭打ち。**厳密な 120 秒の天井ではない**が、この裾は #300 以前から存在した検証コストそのもので、今回の変更が増やしたものではない。**天井を締めたいなら検証を削るのではなく短リスト上限（`_maxMeasureShortlist`）を下げること。**
