...
## 7. 実機検証で判明した限界（2026-06-19・重要）

実装後に実機（蒲田付近→上野・制限3時間）で検証したところ、**この変更だけでは徒歩最大化は達成できない**ことが分かった。実装は「乗れない偽ルートを出さない」正しさ改善として価値があるが、**目的（徒歩最大）には別の本修正が要る**。経緯と理由を残す。

### 観測（実機トレース）

- 通過駅フィルタ（§実装）で bogus 候補は減った（試行7→3、田町など停車駅は正しく扱われた）。
- それでも徒歩の多い候補（walk 101〜166分）が**全て再アンカーで脱落**し、`walk=3m・余裕147分`の全電車ルートへ縮退した。

### 根本原因：再アンカーの「徒歩時間の不整合」

例（大森→田町・徒歩166分）：

1. 現在地→大森を48分歩いて 17:05 に大森着。乗り遅れ → 大森から再照会（`notBefore=17:05`）。
2. NAVITIME は **1駅 goal 寄りの大井町発17:11** の快速（田町17:18停車）を返す（route_transit は最速＝歩いて先の駅で乗る経路を返す）。
3. 再アンカーで乗車駅を大井町にすると、徒歩は**現在地→大井町=80分**に伸び、到着は **17:37**。
4. しかし電車は **17:11発** → 80分歩いて着く頃には出た後 → **再アンカー後も乗り遅れ** → 正しく除外。

`notBefore` は「元の乗車駅（大森）に着く時刻」基準だが、NAVITIME が前方の駅から乗る経路を返すため、**現在地からその前方駅まで歩くと間に合わない**。反復再照会しても、前方乗車は徒歩が伸びて予算超過になり詰む。

### 構造的限界

**route_transit は「指定駅を発車する実在列車」を返さない**（常に座標からの最速経路＝前方へ歩かせる）。「手前の駅で乗って大半を歩く」という当アプリの要求と、route_transit の最適化が根本的にミスマッチ。徒歩最大ルートを正しく出すには：

- **(A) 駅時刻表 API**（その駅を発車する列車を直接引く）を使う、または
- **(B) 各停ベースの基準経路**も併用し、停車駅の多い系統で乗降候補を作る、

といった**データ源・候補生成の再設計**が必要。本書の A案/B案（再アンカー・別路線）はその前段の正しさ改善に留まる。

### 本コミットで入れた範囲（正しさ改善として維持）

- `_findRealBoarding` の制約緩和（乗車駅名・路線名を問わない）＋乗車駅情報の返却。
- `_refetchMissedTrain` の再アンカー（NAVITIME の実乗車駅へ walk1 を測り直し）。
- ハイブリッド生成の**通過駅除外**（時刻を持つ区間では dep/arr のある駅のみ乗降可。#67 のデータ欠落区間は従来どおり全許容）。
- 選定ループの**上限到達フォールバック是正**（無検証の乗り遅れルートを返さない）。

→ いずれも「乗れない偽ルートを確定しない」方向の改善で回帰なし（既存テスト緑）。徒歩最大化の本修正は上記 (A)/(B) として別途。
