...
/// 直叩きと同じ 35 秒にしないのは、プロキシの無応答は本当に異常＝早く縮退した方が良いから。
///
/// なお徒歩実測には[検索の締切][searchDeadlineBudget]が**掛からない**（[TransitApiClient]）。
/// 実測は探索の改善ではなく確定経路の検証で、飛ばすと答えが劣化するのではなく嘘になる。
const proxyRequestTimeout = Duration(seconds: 15);

/// 検索の**探索**（Transit 引き直し）に許す時間（#300）。超過後は引き直しを打ち切り、
/// 既得の候補で確定する。
///
/// 正常時の検索は初期 guidance(~10s)＋乗車駅探索の引き直しで 30〜60 秒（§2.4）。120 秒は
/// その上に裾（30 秒級）が数本重なっても引き直しを完走できる幅を残しつつ、探索を止める。
///
/// **検索全体の天井ではない。** 締切が掛かるのは Transit 側だけで、確定経路の徒歩実測は
/// 締切後も走る（§2.4）。一律に掛けると予算超過の経路を「予算内」と偽って確定させる。
