...
本文の器は遷移で作り直されない（替わるのは `<Outlet>` の中身だけ）。この器がスクローラなので、
画面が移ったら `scrollTop` を先頭へ戻す——ブラウザの復元は document のスクロールしか見ない。
...
**開く合図は focus だけでは足りない。** 候補を押しても焦点は欄に残る（押下で blur を
止めているため）ので、二度目に押しても focus が起きない。click も合図にしている。
Enter は一覧が開いているときだけ確定する——閉じても候補と選択位置は残るので、素通しすると
`aria-expanded=false` の欄で見えていない候補が入る（PR #407 の Codex レビュー）。

目的地が未選択のときの home の CTA は、この幅では全画面の検索へ飛ばさず欄へ焦点を移す。
飛ばすと、この幅のために作った導線を CTA が自分で迂回することになる。

欄の輪郭は一覧の開閉ではなく**焦点**に紐づける（`:focus-within`）。入力側の outline を
消してあるので、開いているかどうかだけに紐づけると Escape の直後に焦点の在処が消える。

錠を解くのは、今の世代を持っている確定だけ。追い越された古い確定が解くと、走っている
新しい確定の横で重複した照会を始められ、後から来た本命が世代ずれで捨てられる。

**IME の変換中はキーを1つも横取りしない。** 変換中の ↑↓ は変換候補を選ぶ操作、Enter は
変換の確定で、どちらもこの欄の操作ではない——奪うと日本語が打てない欄になる
（`event.nativeEvent.isComposing`）。jsdom の `isComposing` はこちらが立てた旗にすぎないので、
実際に変換中のキーがどう届くかは CDP の `Input.imeSetComposition` で起こして
`e2e/typeahead.spec.ts` が確かめる。ガードを外すと赤くなることを確認済み。

確定済みの地点は消去ボタンで外せる（移植元の `searchClearInput`。全画面の検索と同じ文言）。
無いと、確定名を手で選択して消すしか戻す手が無く、読み上げから辿れる導線も失われる。

確定中の錠は打ち替えで解く。古い確定は世代で無効化されるので待たせる理由が無く、持ったままだと
新しい候補を押しても無反応な数秒が残る。選択位置は描画のたびに行数へ丸め、確定で先頭へ戻す
——並びは打鍵のたびに入れ替わり、確定では候補から履歴へ替わるので、移動時のクランプだけでは
存在しない行を指せてしまう。選択位置の行は `scrollIntoView({ block: 'nearest' })` で一覧の中へ
送る（一覧は 320px で頭打ち）。jsdom は `scrollIntoView` を実装しないので `test/setup.ts` が
敷いている（matchMedia と同じ扱い）。
...
### 時刻欄は native のまま、5分刻みのステッパーだけ足す

時刻・日付の欄とステッパーは1つの下書きとして扱う。欄の中の移動では確定せず、**3つ全部が**
blur を見る——ステッパーだけ見ていないと、そこへ Tab で入ってから欄の外へ出た打鍵が黙って
捨てられ、検索は古い時刻で走る（PR #407 の Codex レビュー）。

移植元 `desktop_time_field.dart`（427 行）は**作り直さない**。あの欄が自作なのは、Flutter の
web に native の入力が無く、モバイルのホイールシートがデスクトップで使い物にならなかった
ため。`<input type="time">` はその両方を既に満たしている——キー入力・端末に合わせた UI・
読み上げ・範囲（min/max）が付いてくる。自作の parse 規則（数字以外を除去して下2桁を分…）は
生のテキスト欄の都合であって、仕様ではない。

**足りなかったのはマウスで 5 分刻みに動かす手段だけ。** ↑↓ の 5 分刻みと日跨ぎの日送りは
スライス5 以前から入っており（`time-field.tsx` の `onTimeKeyDown`）、ステッパーは同じ
`stepBy` を通す。native のスピナーに預けられない理由は2つ:

- 分が **1 ずつ**動く（移植元・ハンドオフとも 5 分）
- **同日内で折り返す**。23:58 から進めても翌日にならず 00:00 になり、翌日のままの到着との
  予算が 25 時間近くへ膨らむ

`step` 属性で 5 分刻みにする手も採らない。倍数でない時刻（12:03 など）が `:invalid` になる
——打てる値を制約するためのものではない。

**帰結として、時刻の表示形式は利用者のブラウザのロケールに従う。** 日本語のブラウザなら
24 時間表記、英語圏の設定なら `11:16 PM` になる（Playwright の Chromium がまさにそれ）。
Flutter 版は常に 24 時間表記だった。表示を固定するには欄そのものを自作に戻すことになるので、
ここでは端末の慣習に従う側を採っている。
...
`100dvh` のままだと、地図はバーの高さぶん下へはみ出す——シェルが `--screen-min-height` を
差し替えているから合っている。上流をわざと遅らせて待ち画面に留め、地図の矩形が
「バーの真下からビューポートの下端まで」であることを `desktop-layout.spec.ts` で固定した。
