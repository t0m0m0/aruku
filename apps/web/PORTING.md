...
**直していない。** History API は履歴のエントリを削除できず、前進側を切り詰めるには push が要る
——つまり必ず1つ積むことになり、`[home]` だけを残す終わり方が存在しない。ガードは弾いた位置を
どこか妥当な場所に着地させねばならず、その着地点が home である以上、真下が home なら重複する。
代償は「戻る」を1回余分に押すこと。移植元は Navigator スタックを直接持てたので同じ制約が無い。
