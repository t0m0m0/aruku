...
| 語彙の出どころ | コメントを剥いだ全 `.ts(x)` の文字列リテラルと `index.html` | `i18n/ja.ts` だけでは足りない。`format.ts` の「月」「日」と `loading-screen.tsx` の「まで · 制限」が抜ける |
...
| --- | ---: |
| JS | 160.9 KB (gzip) |
| CSS（両段の @font-face 宣言込み） | 40.2 KB (gzip) |
| Fontsource をそのまま引いた場合のフォント | 474 KB |
| 語彙ぶんへ絞った場合のフォント（25 ファイル） | 126 KB |
...
完了条件は満たしたまま（最悪でも 327 KB + 80 KB = 407 KB < 500 KB）なので、このスライスでは
