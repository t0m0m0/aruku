...
**作り物の地図は viewBox を固定して `slice` で埋める。** 移植元は実ピクセルの `Size` を受け取り、
割合で置く図形（公園・道路）と絶対寸法で置く図形（建物 26x22、ピンの半径、線の太さ）を
混ぜていた。SVG で割合のまま伸縮させると、横長の枠で建物とピンだけが引き伸ばされて潰れる。
