...
前提ルール（`CLAUDE.md` と `.claude/docs/` が正）:
...
`.claude/agents/worker.md` の流儀に従う:
...
PR本文には次を明記する（`.claude/docs/workflow.md`）:

- What changed / Why / Test coverage
- `Closes #<issue番号>`
...
```
git commit -m "fix(#ISSUE): レビュー指摘対応 - <要点>"
git push
```
