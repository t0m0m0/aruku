...
鍵は Secret `RATE_LIMIT_HMAC_KEY` と UTC 日付から導出され日次でローテーションするため、万一
Firestore ダンプが流出しても生 IP は復元できず、日を跨いだ IP 相関もできません（Issue #263）。
...
# 2. IP ハッシュ化用の HMAC 鍵を登録（functions デプロイ前に必須）。
#    ランダムな十分長い値を設定する。未設定でも起動は落ちないが、開発用
#    フォールバック鍵に丸まりハッシュが公開値となるため本番では必ず登録する。
